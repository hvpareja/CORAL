...
## Remote Runtime State Bridge

Use `agents.remote_runtime` when an agent executes outside the local CORAL
worktree but you still want `coral status` and the web dashboard to show its
state. The adapter is additive: local subprocess agents keep using
`agents.runtime`, and the remote adapter only syncs state into
`.coral/public/remote_state/`.

```yaml
agents:
  remote_runtime:
    class: my_package.remote:Runtime
    config:
      endpoint: https://example.internal
    sync_interval_seconds: 30
```

The class must implement `coral.agent.remote.RemoteRuntime`:
